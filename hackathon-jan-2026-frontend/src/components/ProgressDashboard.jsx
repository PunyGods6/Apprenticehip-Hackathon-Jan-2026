...
function ProgressDashboard({ totalOTJHours, weeklyTarget, totalTarget, entries }) {
  const progressPercentage = Math.min((totalOTJHours / totalTarget) * 100, 100);
  const variance = totalOTJHours - weeklyTarget;
  ...
  return (
    <div className="progress-dashboard">
      <div className="dashboard-cards">
        ...
        <div className={`stat-card ${variance >= 0 ? 'success' : 'warning'}`}>
          <div className="stat-icon">{variance >= 0 ? '✅' : '⚠️'}</div>
          <div className="stat-content">
